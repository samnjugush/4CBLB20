...
#define S_FUNCTION_NAME ec_IposResetEnc_v2
...
#include <math.h>
extern int ec_IPOS_enc_read(double *, int);
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
       ...
       real_T *yencM1    =   ssGetOutputPortRealSignal(S,0);
...
//        ichan=(int_T) (*(mxGetPr(CHAN_ID)));
       ...
       /* INPUTS*/
if U0(0){
    ec_IPOS_enc_read(&yencM1[0],0);
    for (int i=0; i<4 ;i++) {
           iret = ec_IPOS_enc_reset_chan(i);
           printf("Counter: %d \n", i);
    }
//           printf("We zijn hier");
           if (iret != 0){
