...
% Last Modified by GUIDE v2.5 06-Feb-2015 09:53:59
...
%   set active channel settings
    ...
    g.QSG_GEN_CHAN(g.QSG_GEN_SIGNAL)=tde_gate_m(def.CALL_FG_STATUS, g.QSG_GEN_SIGNAL-1, g.QS_IP);
%   type    
    type=tde_gate_m(def.CALL_FG_GET_TYPE, g.QSG_GEN_SIGNAL-1, g.QS_IP);
...
%   power buttons
    ...
    if g.QSG_GEN_CHAN(g.QSG_GEN_SIGNAL)==1;
        %fix color: set(handles.tag_gen_power,'Backgroundcolor',def.color_YELLOW);
        set(handles.tag_gen_power,'FontWeight','normal', 'Backgroundcolor','green');
    else
        set(handles.tag_gen_power,'Backgroundcolor',def.COLOR_GREY);
    end

    onoff=tde_gate_m(def.CALL_FG_GET_ONOFF, g.QS_IP);
    set(handles.tag_gen_onoff,'Value',onoff);
    if onoff==1
        %fix color: set(handles.tag_gen_onoff,'Backgroundcolor',def.color_YELLOW);
        set(handles.tag_gen_onoff,'FontWeight','normal', 'Backgroundcolor','green');
...
%   offset slider and display
    ...
    offset=tde_gate_m(def.CALL_FG_GET_OFFSET, g.QSG_GEN_SIGNAL-1, g.QS_IP);
...
%   amplitude slider and display
    ...
    amp=tde_gate_m(iget, g.QSG_GEN_SIGNAL-1, g.QS_IP);
...
tde_gate_m(def.CALL_FG_SET_TYPE, type, g.QSG_GEN_SIGNAL-1, g.QS_IP);
...
amp=tde_gate_m(ifun, g.QSG_GEN_SIGNAL-1, g.QS_IP);
set(handles.tag_gen_amplitude,'Value',amp);
set(handles.tag_gen_amplitude_disp,'String',num2str(amp));
...
tde_gate_m(def.CALL_TDE_SET_DAC_STATE, istate, g.QSG_GEN_SIGNAL-1, g.QS_IP);

if istate==1
    g.QSG_GEN_CHAN(g.QSG_GEN_SIGNAL)=1;
    tde_gate_m(def.CALL_FG_START, g.QSG_GEN_SIGNAL-1, g.QS_IP);
    %fix color: set(handles.tag_gen_power,'Backgroundcolor',def.color_YELLOW);
    set(handles.tag_gen_power,'FontWeight','normal', 'Backgroundcolor','green');

else
    g.QSG_GEN_CHAN(g.QSG_GEN_SIGNAL)=0;
    tde_gate_m(def.CALL_FG_STOP, g.QSG_GEN_SIGNAL-1, g.QS_IP);
    set(handles.tag_gen_power,'Backgroundcolor',def.COLOR_GREY);
...
tde_gate_m(def.CALL_FG_SET_OFFSET, offset, g.QSG_GEN_SIGNAL-1, g.QS_IP);
set(handles.tag_gen_offset_disp,'String',num2str(offset));
...
tde_gate_m(def.CALL_FG_SET_OFFSET, 0, g.QSG_GEN_SIGNAL-1, g.QS_IP);
...
tde_gate_m(ifun, amp, g.QSG_GEN_SIGNAL-1, g.QS_IP);
set(handles.tag_gen_amplitude_disp,'String',num2str(amp));
...
if type==def.TWOTONE
    ...
    case 1,
        amp=tde_gate_m(def.CALL_FG_GET_AMP1, g.QSG_GEN_SIGNAL-1, g.QS_IP);
        set(handles.tag_gen_amp_select,'String','1');
    case 2,
        amp=tde_gate_m(def.CALL_FG_GET_AMP2, g.QSG_GEN_SIGNAL-1, g.QS_IP);
        set(handles.tag_gen_amp_select,'String','2');
    end
...
%   update gen frequency 1 setting
    tde_gate_m(def.CALL_FG_SET_FREQ1, freq,g.QSG_GEN_SIGNAL-1, g.QS_IP);
case 2,
%   update gen frequency 2 setting
    tde_gate_m(def.CALL_FG_SET_FREQ2, freq,g.QSG_GEN_SIGNAL-1, g.QS_IP);
end
...
if type==def.TWOTONE | type==def.CHIRP | type==def.MULTISINE | type==def.WHISTLE
    ...
    switch g.QSG_GEN_FREQ_SELECT
    case 1,
        freq=tde_gate_m(def.CALL_FG_GET_FREQ1, g.QSG_GEN_SIGNAL-1, g.QS_IP);
        set(handles.tag_gen_freq_select,'String','1');
    case 2,
        freq=tde_gate_m(def.CALL_FG_GET_FREQ2, g.QSG_GEN_SIGNAL-1, g.QS_IP);
...
tde_gate_m(def.CALL_FG_SET_ONOFF, onoff, g.QS_IP);
...
tde_gate_m(def.CALL_TDE_LINK2MEAS, ilinked, g.QS_IP);
...
if ilinked==1
    ...
    set(handles.tag_gen_onoff,'Backgroundcolor','red');
else
...
hold_order=tde_gate_m(def.CALL_FG_GET_HOLD_ORDER, g.QSG_GEN_SIGNAL-1, g.QS_IP);
...
tde_gate_m(def.CALL_FG_SET_HOLD_ORDER, hold_order, g.QSG_GEN_SIGNAL-1, g.QS_IP);
...
%	AQI
	    ...
	    set(g.QSG_HANDLES.tag_gen_hoh,'String',num2str(hold_order));
	    %fix color: set(g.QSG_HANDLES.tag_gen_hoh,'Backgroundcolor',def.color_YELLOW);
	    set(g.QSG_HANDLES.tag_gen_hoh,'FontWeight','normal', 'Backgroundcolor','green');
