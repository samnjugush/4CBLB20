...
% QLGUI Application M-file for qlgui2.fig
%    FIG = QLGUI launch qlgui GUI.
%    QLGUI('callback_name', ...) invoke the named callback.

%% Change Log in glgui.fig
% 04/02/2015: grouped gui items in panels [MPlantinga]
% 04/02/2015: changed save & open text buttons to icons [MPlantinga]
% 04/02/2015: added TU/e and QS logos [MPlantinga]

%% change log in glgui.m
...
%   Generate a structure of handles to pass to callbacks, and store it. 
    ...
    act_pos=get(gcf,'Position');
    xr=scrsz(3)-act_pos(3);
    xp=round(xr/2);
    yr=scrsz(4)-act_pos(4);
    yp=round(yr/2);
    set(gcf,'Position', [xp yp act_pos(3) act_pos(4)]);
...
%   disable all channels
    for i=1:2*g.QS_NDEVS,
        tde_gate_m(def.CALL_TDE_SET_ADC_STATE, 0, i-1, g.QS_IP);
        g.QS_CHAN_IN_STATE(i)=tde_gate_m(def.CALL_TDE_GET_ADC_STATE, i-1, g.QS_IP);
    end
    for i=1:2*g.QS_NDEVS,
        tde_gate_m(def.CALL_TDE_SET_ENC_STATE, 0, i-1, g.QS_IP);
        g.QS_CHAN_IN_STATE(g.QS_MAXCHAN+i)=tde_gate_m(def.CALL_TDE_GET_ENC_STATE, i-1, g.QS_IP);
    end
    for i=1:2*g.QS_NDEVS,
        tde_gate_m(def.CALL_TDE_SET_DAC_STATE, 0, i-1, g.QS_IP);
        g.QS_CHAN_OUT_STATE(i)=tde_gate_m(def.CALL_TDE_GET_DAC_STATE, i-1, g.QS_IP);
    end

%   enable first analog in channel on scope 1
    tde_gate_m(def.CALL_TDE_SET_ADC_STATE, 1, 0, g.QS_IP);
    g.QS_CHAN_IN_STATE(1)=tde_gate_m(def.CALL_TDE_GET_ADC_STATE, 0, g.QS_IP);
    g.QS_SCOPE_CHAN(1)=1;

%   no offsets
    for i=1:2*g.QS_NDEVS,
        tde_gate_m(def.CALL_TDE_SET_ADC_OFFSET, 0.0, i-1, g.QS_IP);
	g.QS_CHAN_IN_OFFSET(i)=tde_gate_m(def.CALL_TDE_GET_ADC_OFFSET, i-1, g.QS_IP);
    end
    for i=1:2*g.QS_NDEVS,,
%       tde_gate_m(def.CALL_TDE_SET_DAC_OFFSET, 0.0, i-1, g.QS_IP);
%	g.QS_CHAN_OUT_OFFSET(i)=tde_gate_m(def.CALL_TDE_GET_DAC_OFFSET, i-1, g.QS_IP);
...
%	    AQI/MicroGiant has variable range
    	    g.QS_CHAN_IN_RANGE(i)=tde_gate_m(def.CALL_TD_GET_RANGE, g.QS_CHAN_SIGNAL-1, 0, g.QS_IP)+1;
	end
    end
...
%   selected channel is of type adc
    g.QS_CHAN_IN_STATE(g.QS_CHAN_SIGNAL)=tde_gate_m(def.CALL_TDE_GET_ADC_STATE, g.QS_CHAN_SIGNAL-1, g.QS_IP);
    g.QS_CHAN_IN_OFFSET(g.QS_CHAN_SIGNAL)=tde_gate_m(def.CALL_TDE_GET_ADC_OFFSET, g.QS_CHAN_SIGNAL-1, g.QS_IP);
else
%   selected channel is of type enc
    g.QS_CHAN_IN_STATE(g.QS_CHAN_SIGNAL)=tde_gate_m(def.CALL_TDE_GET_ENC_STATE, g.QS_CHAN_SIGNAL-1-g.QS_MAXCHAN, g.QS_IP);
end
...
if g.QS_CHAN_SIGNAL<=g.QS_MAXCHAN
    tde_gate_m(def.CALL_TDE_SET_ADC_STATE, istate, g.QS_CHAN_SIGNAL-1, g.QS_IP);
    g.QS_CHAN_IN_STATE(g.QS_CHAN_SIGNAL)=tde_gate_m(def.CALL_TDE_GET_ADC_STATE, g.QS_CHAN_SIGNAL-1, g.QS_IP);
else
    tde_gate_m(def.CALL_TDE_SET_ENC_STATE,istate, g.QS_CHAN_SIGNAL-1-g.QS_MAXCHAN, g.QS_IP);
    g.QS_CHAN_IN_STATE(g.QS_CHAN_SIGNAL)=tde_gate_m(def.CALL_TDE_GET_ENC_STATE, g.QS_CHAN_SIGNAL-1-g.QS_MAXCHAN, g.QS_IP);
end
...
% Offset function for offset slider
...
tde_gate_m(def.CALL_TDE_SET_ADC_OFFSET,offset, g.QS_CHAN_SIGNAL-1, g.QS_IP);
g.QS_CHAN_IN_OFFSET(g.QS_CHAN_SIGNAL)=tde_gate_m(def.CALL_TDE_GET_ADC_OFFSET, g.QS_CHAN_SIGNAL-1, g.QS_IP);
...
%   if g.QS_IS_SIMULINK~=1
    tde_gate_m(def.CALL_TDE_INIT_ENC, g.QS_CHAN_SIGNAL-1-g.QS_MAXCHAN, g.QS_IP);
%   end
end
...
g.QS_CHAN_SIGNAL=get(handles.tag_chan_signal,'Value'); % value goes from 1 to 9, starting with +/- 10V
...
if g.QS_CHAN_SIGNAL<=g.QS_MAXCHAN
    ...
    tde_gate_m(def.CALL_TD_SET_RANGE,irange-1, g.QS_CHAN_SIGNAL-1-ioff, ilink, g.QS_IP);
    g.QS_CHAN_IN_RANGE(g.QS_CHAN_SIGNAL)=tde_gate_m(def.CALL_TD_GET_RANGE, g.QS_CHAN_SIGNAL-1-ioff, ilink, g.QS_IP)+1;
...
%   	synchronize hardware with restored settings
	for i=1:2*g.QS_NDEVS,
	    tde_gate_m(def.CALL_TDE_SET_ADC_STATE,g.QS_CHAN_IN_STATE(i), i-1, g.QS_IP);
        g.QS_CHAN_IN_STATE(i)=tde_gate_m(def.CALL_TDE_GET_ADC_STATE, i-1, g.QS_IP);
	end
	for i=1:2*g.QS_NDEVS,
        tde_gate_m(def.CALL_TDE_SET_ENC_STATE,g.QS_CHAN_IN_STATE(g.QS_MAXCHAN+i), i-1, g.QS_IP);
	    g.QS_CHAN_IN_STATE(g.QS_MAXCHAN+i)=tde_gate_m(def.CALL_TDE_GET_ENC_STATE, i-1, g.QS_IP);
	end
%	for i=1:2*g.QS_NDEVS,
%    	tde_gate_m(def.CALL_TDE_SET_DAC_STATE,g.QS_CHAN_OUT_STATE(i), i-1, g.QS_IP);
%	    g.QS_CHAN_OUT_STATE(i)=tde_gate_m(def.CALL_TDE_GET_DAC_STATE, i-1, g.QS_IP);
%	end
	g.QS_CHAN_OUT_STATE=bak_g.QS_CHAN_OUT_STATE;
%   offsets
	for i=1:2*g.QS_NDEVS,
        tde_gate_m(def.CALL_TDE_SET_ADC_OFFSET,g.QS_CHAN_IN_OFFSET(i), i-1, g.QS_IP);
	    g.QS_CHAN_IN_OFFSET(i)=tde_gate_m(def.CALL_TDE_GET_ADC_OFFSET, i-1, g.QS_IP);
	end
	for i=1:2*g.QS_NDEVS,,
%       tde_gate_m(def.CALL_TDE_SET_DAC_OFFSET,g.QS_CHAN_OUT_OFFSET(i), i-1, g.QS_IP);
%	    g.QS_CHAN_OUT_OFFSET(i)=tde_gate_m(def.CALL_TDE_GET_DAC_OFFSET, i-1, g.QS_IP);
	end
...
%   	range of adc channels
	for i=1:2*g.QS_NDEVS,
	    ...
	    if g.QS_AQI(ilink+1)==1
            tde_gate_m(def.CALL_TD_SET_RANGE,g.QS_CHAN_IN_RANGE(i)-1, g.QS_CHAN_SIGNAL-1, 0, g.QS_IP);
    		g.QS_CHAN_IN_RANGE(i)=tde_gate_m(def.CALL_TD_GET_RANGE, g.QS_CHAN_SIGNAL-1, 0, g.QS_IP)+1;
...
%	irange(i)=1;
%    else
	irange(i)=tde_gate_m(def.CALL_TD_GET_RANGE, i-1-ioff, ilink, g.QS_IP)+1;
%    end
end
...
for i=1:g.QS_NDEVS,
    if (g.QS_AQI(i)==1)
        if iconfig==0
            tde_gate_m(def.CALL_TD_AAFILTER_OFF, i-1, g.QS_IP);
        else
            tde_gate_m(def.CALL_TD_AAFILTER_ON, fs/2.5, iconfig, i-1, g.QS_IP);
        end
    end
end
